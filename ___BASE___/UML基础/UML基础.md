<center><B><BBBG>UML基础</BBBG></B></center>
...
UML其实有<B>很多种</B>图的形式，但是目前无需关心太多，我所关注的是<B><GN>类图Class Diagram</GN></B>
...
<B><GN>泛化</GN></B>指的是<B>派生关系</B>，要注意箭头关系，是<B><VT>子类指向父类</VT></B>
![](Pic/泛化.png)
<B><GN>实现</GN></B>指的是<B>接口的实现</B>，同样要注意箭头关系，是<B><VT>类指向接口(某种意义上也是子类指向父类)</VT></B>
![](Pic/实现.png)

<B><GN>关联</GN></B>本质上指的是<B>两者具有联系</B>
关联本身也是一种关系，<B><VT>仅表示两者具有关联</VT></B>，如下：
![](Pic/关联.png)
<B><BL>问题：如上图所示，并没有表现出是谁吃谁</BL></B>
<BL>虽然上图中没有体现，但是如果添加函数后即可明确，显然Cat内肯定会有一个`Eat()`</BL>
关联<B>细分</B>有两种情况，即组合聚合：

- <B><GN>组合</GN></B>
  组合指的是<B>强关联</B>，父节点的存亡会影响子节点(父Destroy子也会Destroy)
  ![](Pic/组合.png)
  可以看到是<B><VT>子指向父</VT></B>
- <B><GN>聚合</GN></B>
  ![](Pic/聚合.png)
  聚合指的是<B>弱关联</B>，父节点的存亡不影响子节点

  可以看到同样是<B><VT>子指向父</VT></B>

对于关联的两种情况，都可以<B>标注更多信息</B>，即<B><GN>多重性Multiplicity</GN></B>：
![](Pic/组合聚合多重性.png)
<YL><B>对于这个例子来说：</B>
...
<B><GN>依赖</GN></B>即<B>一种依赖关系</B>，是<B><VT>依赖者指向依赖物</VT></B>　　<VT>简单来说就是：<B>"用到"</B></VT>
![](Pic/依赖.png)

从以上几种关系中我们能够得出一个<B>结论</B>：
<B><VT>箭头的方向永远是子指向父</VT></B>

---

## 类

对于类图来说，最关键的当然是<B>类</B>，那么<B><VT>类中到底有什么我们应该在图中表示出来</VT></B>，就像这样：
![](Pic/Person类.png)
可以看到类被分成了<B>3个部分</B>，从上到下依次为：

- <B>类名</B>
- <B>属性</B>Attributes/Fields/Variables/Properties
  这里的属性包括了字段与属性
- <B>函数</B>Methods/Functions/Operations
...
![](Pic/VP2.png)
填入Name/Data type set即可，这里比较重要的是<B>Data type set</B>，我们需要选择相应的语言，这决定了<B><VT>后续类型</VT></B><YL>(如：C#是string，Java是String)</YL>

<B>2.图的添加</B>
此时项目本身已经创建完毕，但是这只是一个空项目没有图，所以需要添加图
有2种方式：

- 可以看到<B>初始界面</B>有一<B>快捷创建方式</B>，使用即可
  对于UML类图，依次选择UML->Class Diagram->Blank->输名字即可
- 标准流程为点击上侧菜单View按钮，在Panes种找到<B>Diagram Navigator</B>调出
  ![](Pic/VP3.png)
...
由上述内容我们也能发现：
<B><VT>View的Panes其实就是一般软件中的Windows</VT></B>
...
- <B>由于`instance`变量是static的，所以需要加上底线</B>，方法如下：
  ...
  - <B>Type Modifier</B>：类型修饰符，即<B><VT>额外的类型数据</VT></B>
  ...
  - <B>Setter/Getter</B>：Get/Set方法或属性<VT>(属性就是一种Get/Set方法)</VT>
    <B><VT>默认情况下没有表示</VT></B>，<B>显示方法</B>：
...
- 对于<B>class</B>来说，同样具有<B>Model Element Properties</B>，具体如下：
...
- <B>后缀型Type Modifier</B>
  ...
  - 泛型`<>`
    - 无约束：`<T>`
    - 有约束：`<约束类型>`
    <B><DRD>注意：编写时应该先不处理Type Modifier，然后通过选中Enter进行修改，否则会附着到Return Type</DRD></B>
...
- <B>前缀型Stereotypes</B>
  ...
  要注意的是<B><VT>默认没有</VT></B>这些内容的，所以需要<B>自行添加</B>：
  ![](Pic/VP14.png)

### 功能备注

<B>这里稍微记录一下我发现的VP里的功能：</B>
...
<B><DRD>重要：将所有声明完整展示并不有效</DRD></B>
![](Pic/UML实践1.png)
就以最核心的`IArchitecture`为例，我并不需要知道`RegisterSystem()`需要一个ISystem类型的system，知道有这个操作足以，如果真的关心<B>看源码</B>的即可，同样的`SendCommand()`的重载也没什么必要，<B><VT>个人认为更多的需要的是一点Note来快速提示类/接口的核心</VT></B>
