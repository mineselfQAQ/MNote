<center><B><BBBG>TortoiseSVN使用</BBBG></B></center>
...
<B><BL>问题：为什么Edit log message操作不被允许</BL></B>
...
<B>ignore与global-ignores的区别如下：</B>

- ignore：处理目录特定的忽略需求
- global-ignores：处理项目范围内通用的忽略模式

<B>Apply property recursively</B>作用：<B><VT>开启属性递归(可作用于子文件夹)</VT></B>

<B><VT>一般选择：ignore+开启Apply property recursively</VT></B>

<B>注意：</B>
<B><DRD>此时如果已经提交过内容且存在文件夹，发现有修改是正常的(针对递归情况)，因为是文件夹规则发生变动</DRD></B>
![](Pic/SVN3.png)
