...
- [Jobs101](Jobs101/Jobs101.md)：从普通 MonoBehaviour 逐步迁移到 Burst + Job，并引入并行和排序优化。
- [Entities101](Entities101/Entities101.md)：Entities API 入门主线，覆盖 Baking、System、IJobEntity、IJobChunk、Prefab、SubScene、状态变化和空间查询。
- [Physics101](Physics101/Physics101.md)：Unity Physics 入门样例，覆盖碰撞、触发、查询、关节式玩法、浮力和刚体控制。
- [Netcode101](Netcode101/Netcode101.md)：Netcode for Entities 的 Kickball 入门版本，展示 client/server world、ghost、输入同步和预测式玩法基础。
- [ContentManagement101](ContentManagement101/ContentManagement101.md)：Content Management API 入门，重点是 weak object / weak scene reference 和本地、远程内容加载。
- [DotsUI](DotsUI/DotsUI.md)：UI Toolkit 与 Entities 的集成样例，展示 ECS 游戏逻辑如何驱动 HUD、背包、对话和任务 UI。

---

## 学习路线

1. 先看 [Jobs101](Jobs101/Jobs101.md)，理解数据搬进 `NativeArray`、Job 调度、Burst 编译和依赖关系。
2. 再看 [Entities101](Entities101/Entities101.md)，把 Job 思维接到 Entity、Component、System、Baking 和 SubScene。
3. 接着看 [Physics101](Physics101/Physics101.md)，理解 Unity Physics 在 ECS 世界里的组件化输入和系统更新方式。
4. 然后看 [Netcode101](Netcode101/Netcode101.md)，理解多人世界拆分、输入、RPC/ghost 的基本模型。
5. 再看 [ContentManagement101](ContentManagement101/ContentManagement101.md)，补上运行时内容加载和场景引用。
6. 最后看 [DotsUI](DotsUI/DotsUI.md)，理解 UI Toolkit 如何通过事件实体和 singleton 与 ECS 逻辑协作。
