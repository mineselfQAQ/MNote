<center><B><BBBG>1_基础</BBBG></B></center>
...
<B>一些知识点：</B>
- <B><GN>chunk</GN></B>---程序块，为<B>一连串的语句或命令</B>
- Lua<B>区分大小写</B>
- 未初始化的变量可以直接访问，为nil
- 不添加前缀就是<b><GN>全局变量</GN></b>，添加local前缀就是<b><GN>局部变量</GN></b>
- 索引起始值为1(否则某些机制无法使用)
- `a = a or 0`，一种<B>默认值</B>的设置方式
- `zip = (address or {}).zipcode`，一种<B>类的安全访问</B>的方式
- Lua支持多重赋值，如`a, b = 10, 2*10`　所以：`a,b,c = 0`是不对的，应该为`a,b,c = 0,0,0`
- `x, y = y, x`，一种<B>交换</B>的方式　　<B><VT>核心：多重赋值+先求值再赋值</VT></B>
- 通过`do ... end`添加块　　<VT>类似与C#的`{ ... }`</VT>
  由此可以创建出<B>不污染全局的局部变量</B>：
  <BR>
...
<B><DRD>注意：从Lua5.3开始，number非拆分成了integer(整型)与float(浮点型)</DRD></B>
在5.3之前：
所有数值都是<B>双精度浮点格式</B>
在5.3之后：
integer-<B>64位整型</B> | float-<B>双精度浮点类型</B>
...
- <B>整型转浮点型</B>可以通过<B>+0.0</B>　　　<YL>如：`-3+0.0`为`-3.0`</YL>
- <B>浮点型转整型</B>，有两种方式：按位或 | `math.tointeger()`
...
<B>优先级</B>如下所示：　　<B><VT>优先级从高到低</VT></B>
![优先级](Pic/优先级.png)
<B>结合律</B>如下所示：
<B>除了`^`和`..`与一元运算符为右结合，其余皆为左结合</B>
...
- <B>string转其它类型</B>---toXXX()　　<YL>如：`tonumber()`</YL>
- <B>其它类型转string</B>：
  - `tostring()`
  - `xx .. ""`　　<VT>即<b>添加空字符串</b></VT>

对于string来说，有<B>2个库</B>：

- <B><GN>string库</GN></B>---<VT>对ASCII操作</VT>
- <B><GN>utf8库</GN></B>---<VT>对utf8操作</VT>　<B>lua5.3新增</B>
  
<B><DRD>注意：string库的内容还是可以用于utf8字符串的，只是形式有所变化(如：`string.len()`会以字节为单位)</DRD></B>

---

## table

table具有<B>唯一的创建方式</B>，即`{}`
具体来说有：
...
我们可以认为1/2形式其实是3形式的<B>语法糖</B>

<B>语法糖：</B>`a["name"] == a.name`　　<B><DRD>注意：是a["name"]而非a[name]</DRD></B>

<B><VT>问题：如何以索引0开始</VT></B>
`{[0] = 0, 1, 2, 3, 4}`　　<B><DRD>注意：极其不推荐，因为有些内容需要起始索引为1的特性</DRD></B>

<B>Tip：分割不仅可以是`,`，还可以是`;`</B>，在某些时候可以起到小分割与大分割的作用
<B><YL>如：</YL></B>`t = {x=10, y=45; "one", "two", "three"}`
...
一般函数的形式为：`print("OK")`
但是存在一种<B>特例</B>：<B><VT>如果函数只有一个参数(必须是<DRD>字面字符串或table构造式</DRD>)，那么可以省略括号(指的是调用时只有一个)</VT></B>
<B><YL>如：</YL></B>`print("OK")`--->`print "OK"`　`require("a")`--->`require "a"`

在Lua中，<B>参数列表是不需要匹配</B>的：
...
<B>关键点：<VT>如果函数调用不是表达式的最后一个元素，那么只产生一个值(对于table初始化来说同理)</VT></B>
...
<B><BL>问题：为什么可以是`ipairs{...}`，不应该是`ipairs(table)`吗</BL></B>
<BL>在前面我们提到过如果只有一个参数是可以省略括号的，那么本质上这其实是`ipairs({...})`，也就是<b>构建了一个table传入</b>，那么我们也能发现：<B><VT>`ipairs`其实是一个函数</VT></B></BL>
...
所以说：<B>与其说是具名实参，不如说这是唯一用法，一种用table包装的方式</B>
...
<B><DRD>注意：如果是商业项目，那么加载程序块时需要报告错误，如果代码不可信任的话，需要在保护环境下执行</DRD></B>

大致上来说，<B>`dofile()`</B>是这样<B>实现</B>的：
...
- `dostring()`并不存在，但我们可以通过`load(s)()`实现　　<B><VT>即用即弃</VT></B>
  由于可能发生错误，所以应该这样：`assert(load(s))()`

### 预编译

根据前面所述，Lua自然可以进行<B>预编译</B>生成<GN><B>预编译文件</B></GN>(即<GN><B>二进制文件binary chunck</B></GN>)
<B>方法</B>大致为：`luac -o prog.lc prog.lua`　将`prog.lua`编译为`prog.lc`
<B><VT>对于预编译生成的.lc文件，是可以被`loadfile()`与`load()`所接收的</VT></B>
<B>有：</B>
...
<B>`string.dump()`---<VT>传入函数，输出字符串</VT></B>

<B>预编译可添加选项</B>
如<B>添加`-l`</B>，则会生成<GN><B>操作码opcode</B></GN>
...
- <B>更快</B>
- <B>避免意外修改源码</B>
  但是同时由于是预编译代码，可能会<B>发生恶意损坏或恶意构造</B>，我们也<VT><B>应该避免运行非受信代码</B></VT>
  这通过`load()`的<B>第3个参数</B>即可<VT><B>限制运行类型</B></VT>：
    - `"t"`---普通代码段
    - `"b"`---预编译(二进制)代码段
    - `"bt"`---两者都可
...
- <B>`error()`---<VT>直接报错</VT></B>
  ...
  其中，可选参数2为<B>level</B>，即<VT><B>报错层级</B>(默认为1，指当前函数)</VT>

- <B>`assert()`---<VT>判断真假并报错</VT></B>
  <BR>
...
<B>异常处理</B>通常有两种<B>方式</B>：

- 返回错误码(nil/false/...)
- 调用`error()`

<B>一种指导原则：<VT>容易避免的异常应该引发错误，否则应该返回错误码</VT></B>
...
对于`io.open()`我们会想到一个问题：<B><BL>如果文件不存在怎么办</BL></B>
...
最简单的话其实就是<B>不处理</B>：`file = assert(io.open(name, "r"))`

### pcall()

如果说`assert()`之类的函数是<B>抛出异常</B>，那么<GN><B>pcall(protected call)</B></GN>则为<B>捕获异常</B>：
<B><VT>对于pcall，不会引发错误，而是会返回bool与错误信息</VT></B>
...
<B><VT>Tip：错误信息不一定是string，可以是任意类型</VT></B>
...
<B><VT>对于错误信息为string的情况下，Lua会尝试传递一些额外信息：</VT></B>
...
pcall存在一定的<B>延迟性</B>，当我们去查看pcall返回的错误信息时，<VT>调用栈已经不完整了</VT>，
我们需要使用<B>另一种形式xcall()</B>进行：　　<B><VT>会在调用<GN>栈展开(unwind)</GN>前调用错误信息函数，则可以使用debug库</VT></B>
...
一般来说，我们可能会用<B>CSV/XML/JSON</B>之类的形式进行持久化存储，
但一种比较特殊的情况就是<B>Lua内置</B>情况，对于该形式即<VT><B>自定义持久化储存，具体逻辑都由我们来决定</B></VT>
...
两种方法都是<B>可行的</B>，但是很明显<VT><B>方法2更加清晰</B></VT>

### 序列化

以上只是一种极易的情况，属于是"比较特例"<VT>(我们读取的并非string/byte数据，而是lua)</VT>
更一般地，我们会需要<B>将数据转换为string/byte真正存储在文件中</B>

<B>具体如下：</B>

#### 一般数据

对于<B>number/string/boolean/nil</B>来说，它们只需使用<B>`string.format()`</B>即可轻松转换：
...
但是它多多少少<VT>影响了原字符串</VT>，如果我们需要<B>保留原始形式</B>则会使用<B><GN>长字符串</GN>的形式</B>
...
对于表来说，有两种可能，即<B>不循环表</B>与<B>循环表</B>，
很明显不循环情况下是非常简单的，大致和其它基本数据类型一致，而对于循环情况处理就较为麻烦

##### 不循环表

一个表可能长这样：`{a = 1, b = 2, 3}`，其实就是一个<B>多数据集合</B>，那么我们<VT>VT</VT>
...
<B><VT>Tip：虽然表情况下有`serialize()`嵌套，但是能处理的只有1层而非循环情况，如果table中遇到kv为table数据，必然是不正确的</VT></B>

##### 循环表

如果带有循环(即递归表)，情况必然复杂许多，<B>具体如下</B>：
...
在这里<B>核心</B>就是通过<VT><B>逐一赋值</B></VT>的方式保证了table的嵌套
...
- 随机函数---`random()` `randomseed()`
  - random函数是<B>均匀分布，伪随机</B>的
    - `random()`：取值范围[0,1)
    - `random(n)`：取值范围[1,n]
    - `random(l,u)`：取值范围[l,u]
  - randomseed函数为<VT><b>random函数计算额外的种子</b></VT>，<B>默认为1</B>
    这会导致每次运算必然得到同一结果，所以<B>解决方法</B>就是：`math.randomseed(os.time())`，这样由于系统时间每次都不同，从而种子不同，运算结果也不同
- `max()` `min()`
- `pi`
- `huge`即<B>inf</B>
...
<B>大致有：</B>　　<VT>表库是一个<b>提供List操作</b>的库</VT>
...
- 排序`sort()`
  - `sort(table)`：排序，<B><VT>默认升序</VT></B>
  - `sort(table, comp)`：根据<B>比较函数comp</B>进行排序　类似于C#的IComparer接口
  - <B><DRD>重要：排序是对table的value进行排序</DRD></B>
...
- <B>简单I/O模型　　<VT>仅能处理单输入输出</VT></B>
  ...
  - `read()`：从当前流读取字符串
    - 格式参数决定了读取的数据：
      - `"a"`---整个文件
      - `"l"`---下一行(丢弃换行符)　　<B>默认参数</B>
      - `"L"`---下一行(保留换行符)
      - `"n"`---一个数值
      - `num`---以字符串读取num个字符
        - `io.read(0)`可以<B>测试是否到达文件末尾</B>，有数据则为空字符串，否则为nil
  - `lines()`：迭代器函数，用于逐行迭代文件<VT>(`read("l")`如果要全读还不如使用`lines()`)</VT>
    - `lines()`：即`io.input():lines()`
    - `lines(f)`：以<B>只读</B>的方式打开对应文件的输入流，达到末尾后<B>自动关闭</B>
    - <B><VT>在lua5.2开始，可以接收`read()`一样的格式参数(如`"a"`)</VT></B>
- <B>完整I/O模型　　<VT>支持多输入输出</VT></B>
  ...
  - `read()` `write()`：读写，<B>在打开文件后进行</B>　<YL>通常为`f:read("a")`(f为流)</YL>　　<VT>Tip：完整模型是<B>针对文件的操作</B>，即`file:xxx()`而非`io.xxx()`</VT>
    - `stdin` `stdout` `stderr`：句柄，用于写入不同的流(输入/输出/错误)，通常是<B>命令行</B>
      - 大致用法为：`io.stderr:write(message)`
    - <VT>Tip：`io.read(args)`是`io.input():read(arg)`的<B>缩写</B></VT>
  - `tmpfile()`：返回一个<B>读写模式的句柄</B>，用于操作临时文件，程序运行结束后会自动删除　　<VT>很像内存流(但不是)</VT>
  ...
  - `seek()`：获取和设置文件的当前位置
    - 参数1`whence`：如何计算偏移　　<B><VT>以字节为单位</VT></B>
      - `"set"`---开头偏移
      - `"cur"`---当前位置偏移　<B>默认值</B>
      - `"end"`---末尾偏移
    - 参数2`offset`：偏移
...
os和io很相近，io为<B>流</B>，而os为<B>真实文件</B>
<B>大概有：</B>
...
- <B>时间相关</B>
  ...
  - 计算相关：
    ...
    - <B>日历机制会导致错误：　　<VT>这是一个与Lua语言无关的问题</VT></B>
...
所以需要调试库进行填补，调试库有两类：
1.<B>自省函数</B>---允许检查运行中的程序的各个内容
2.<B>钩子hook</B>---允许跟踪程序的执行

和一般的反射相同，<B><DRD>调试库必须谨慎使用</DRD></B>，因为会造成<VT>VT</VT>
...
- <B>自省机制</B>
  ...
  - `getlocal([thread,]level,index)`：检查局部函数
    ...
    - 对于<B>变长参数</B>的获取，可以通过<B>负索引</B>来完成
      此时名字会是固定的`(*vararg)`
...
- <B>钩子</B>
  ...
  - `debug()`：即`debug.debug()`，提供了一个能够任意执行Lua语言命令的提示符，其实就是<B><VT>发生错误了，在此处打上断点，通过命令行可进行调试</VT></B>
    - 可能有：
...
该类除了进行调试，还可以进行<B><GN>GN</GN><VT>(资源行为分析<GN>profile</GN>)</VT></B>：
<B><VT>Tip：时间相关调优最好使用C接口(钩子开销太大)，如果是计数性质的话Lua代码即可</VT></B>
