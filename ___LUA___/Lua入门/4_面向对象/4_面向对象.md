...
<GN><B>冒号操作符`:`</B></GN>可以<B>简化self</B>
这是一种<B>语法糖</B>，<VT>使用`:`则会自动将调用者作为第一个参数`self`传入</VT>
- <B>声明</B>时有：`function obj.method(self, arg1, arg2)`将等价于`function obj:method(arg1, arg2)`
- <B>调用</B>时有：`obj.method(obj, 1, 2)`将等价与`obj:method(1, 2)`
...
<B><DRD>Tip：最好不要使用全局变量去存储，容易丢失</DRD></B>
...
在Lua中，本身是<VT><B>没有类的概念</B></VT>的，可以参考<GN><B>基于原型的语言(prototype-ased language)</B></GN>的做法<B>模拟类</B>
<GN><B>基于原型的语言</B></GN>---<VT>对象不属于类，反而每个对象可以有一个原型</VT>

<B>在Lua中的方法：<VT>继承</VT></B>
...
<B><BL>问题：`setmetatable()`中对c进行了设置，后续又进行了c的设置，不冲突吗</BL></B>
<BL>之所以这样是因为：
...
那么对于上述情况也是同理，`c.__index = c`是为了o准备的，而`setmetatable()`是c真正的元表，
所以<B>完整情况</B>为：
...
<B><DRD>注意：这种方法会导致账户无法被GC，直到显式移除</DRD></B>

### 单对象方法

在某些情况下，<B>一个对象只有一个方法</B>，此时我们可以<VT><B>直接返回</B></VT>即可
