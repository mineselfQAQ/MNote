...
在Lua中，<B><VT>GC不能猜测我们认为哪些是垃圾</VT></B>
<YL><B>就比如说栈</B>：栈是通过一个table+一个索引组成的，那么`pop()`操作我们其实只需简单地将索引-1即可，但是这样操作后Lua不可能知道该元素已经"不存在"了</YL>\

所以简单来说：<B>只要还留在数组或存储在全局变量的对象，无论是否会使用，都不会被GC</B>
为了GC，我们将其<B>赋为nil</B>即可<DRD>(全局变量可以，数组不行)</DRD>
...
根据前面提到的内容，其中<B>table比较特殊</B>：
<B><VT>一旦一个对象成为了table的一部分，就再也无法被回收了，因为table在引用它</VT></B>

为了解决这个问题，我们所需的就是<B>弱引用表</B>：
<B><GN>弱引用表</GN>---<VT>用来告知Lua语言一个引用不应阻止对一个对象回收的机制</VT></B>
<B><GN>弱引用</GN>---<VT>一种不在GC考虑范围内的对象引用</VT></B>
...
<B>默认情况</B>下，键值都是<B><GN>强引用</GN>---<VT>会阻止对其所指对象的回收</VT></B>
...
<B>具体来说：<VT>只要有一个键或值被回收了，那么对应的整个键值对都会被从表中删除</VT></B>
...
<B><DRD>注意：只有对象可以被移除，而值是不可回收的
注意：对于字符串来说，从程序员角度看字符串就是值而非对象，无需考虑那么多，是不会被移除的</DRD></B>
...
<B><GN>记忆函数</GN>---<VT>用空间换时间，参数相同返回记忆结果</VT></B>
对于记忆函数，是非常需要弱引用表的
...
可以看到，这其实就是我们非常常用的一种优化方式，就是所谓的<B>缓存表</B><VT>(记忆函数的子集)</VT>

<B>不足之处：</B>
很明显，通过缓存的方式必然能够对于需要多次访问的情况起到加速作用，但是同时带来的就是<B>单次访问的浪费</B>：<VT>如果只需要访问一次存储则是没有意义的</VT>
同时，如果运行时间过久，进行大量的存储，<B>内存</B>迟早会被<B>耗尽</B>
...
但是：<B>如果对象不是table或不想污染元表以上方法就做不到了</B>
<B>解决方案：外部表</B>
...
<B>存在问题：</B>
properties表是<VT>强引用</VT>的，这会<B>导致obj无法被GC</B>
<B>追加解决方案：弱引用表</B>
...
但是强引用必然会导致无法GC，所以<B>改为弱引用</B>肯定是一种更好的方案：
...
我们有时可能会遇到这种问题：<B>一个具有弱引用键的表中的值又引用了对应的键</B>
...
我们能发现工厂函数的<B>问题</B>：<VT>该函数会产生闭包，如果有相同的o，那么这是多余的闭包</VT>
那么解决方案就是<B>记忆方案</B>：
...
此时存在一个值得注意的<B>问题</B>：
此时有<B>弱键o</B>以及<B>强值res</B><VT>(内部引用o)</VT>，我们可能考虑的是`o=nil`之后就可以回收了，但是<VT><B>由于强值res引用o(函数闭包导致)，导致无法GC</B></VT>

但是这是<DRD><B>不会发生</B></DRD>的，因为有瞬表：
<B><GN>瞬表Ephemeron Table</GN>---<VT>具有弱key和强value的表</VT></B>
对于瞬表，即`__mode="k"`情况，Lua会进行<B>特殊处理</B>：
...
在Lua中，析构器是通过<B>元方法`__gc`</B>完成的：　　　<B><DRD>Lua5.1没有</DRD></B>
...
由于<B>复苏</B>的原因，<B>Lua会在2个阶段回收具有析构器的对象</B>：

- 当GC首次发现某个析构器对象不可达，则会将其复苏并放入等待析构队列，一旦析构器指向，该对象就会被标记为已被析构
- 当下一次GC又发现不可达时，会将对象删除

<B><VT>因此想要保证所有垃圾被释放的话，必须调用collectgarbage两次</VT></B>
...
<B>特性：</B>
<B><VT>如果一个对象直到程序运行结束还没有被回收，那么Lua语言就会在整个Lua虚拟机关闭后调用它的析构器</VT></B>
...
<B>特性2：</B>
<B><VT>在GC周期内，GC会在调用析构器前清理弱引用表的值，在调用析构器之后再清理键</VT></B>
即：
...
4次分别是<B>程序运行结束后表的析构(1)，以及每次新表的析构(3)</B>
...
直到Lua5.0，GC使用的是<GN><B>标记-清除式GC</B></GN>，这种方法会时不时停止主程序的运行执行一次完整的GC周期，有四个阶段：<B>标记mark->清理cleaning->清除sweep->析构finalization</B>

- <B>标记阶段</B>---把<GN><B>根节点集合root set</B></GN><VT>(Lua语言可直接访问的对象)</VT>标记为活跃，即C注册表，当所有可达对象都被标记为活跃后，标记阶段完成
- <B>清理阶段</B>---用于处理析构器和弱引用表。会遍历需要析构但没有被标记为活跃的对象，它们会被标记为活跃(复苏)，并放在单独的列表中(在析构阶段用到)，然后遍历弱引用表并从中移除键或值未被标记的元素
- <B>清除阶段</B>---遍历所有对象(通过链表)，没有被标记为活跃的就回收，否则清理标记，准备进行下一GC周期
- <B>析构阶段</B>---调用清理阶段被分离出的对象的析构器

<B>Lua5.1</B>改为<GN><B>增量式垃圾收集器Incremental Collector</B></GN>，在原基础上不再需要停止主程序的运行，会与解释器交替运行，每当解释器分配了一定量的内存时，垃圾收集器也执行一小步
<B>Lua5.2</B>引入<GN><B>紧急垃圾收集Emergency Collection</B></GN>，当内存分配失败时，会强制进行一次完整GC后再尝试分配
...
- `"setpause"`
<B>作用：</B>设置垃圾收集器的 间歇率（pause 参数），控制回收频率。
<B>参数：</B>ata 为百分比值（如 100 表示 1.0）。值越高，GC 越不频繁（内存压力更小）。

- `"setstepmul"`
<B>作用：</B>设置垃圾收集器的 步进倍率（stepmul 参数），控制回收速度。
