...
<B><GN>携程</GN>---<VT>任意时刻只能运行一个协同程序</VT></B>
...
除此以外，Lua携程使用的是<B>非对称携程</B>：
<B><GN>非对称携程Asymmertric Coroutine</GN>---<VT>提供了2个函数控制携程执行(yield/resume)</VT></B>
<B><GN>对称携程Symmetric Coroutine</GN>---<VT>只提供一个函数用于携程切换</VT></B>
...
在携程中，核心操作和Unity的一致，为<B>`yield()`</B>，其大致含义可以理解为<VT>VT</VT>：
...
<B><VT>特性：`resume()`是在<GN>保护模式</GN>下运行的</VT></B>
...
<B><VT>特性：`resume()`与`yield()`会交换数据</VT></B>
...
<B><VT>特性：当携程A唤醒携程B时，A进入特殊状态normal</VT></B>
由于携程是可以自由切换的，即在coA运行的时候进行`coroutine.resume(coB)`，那么此时coA既不是suspended状态，也不是running状态(更不是dead状态)，而是一种<B>normal状态</B>，简单来说就是：<VT>携程被卡在一半不继续进行了</VT>
...
`permutations()`非常常用，所以Lua提供了<B>便捷方法`coroutine.wrap()`：</B>
...
<B><DRD>缺点：缺乏灵活性，无法检查wrap创建的携程的状态已经运行时的错误</DRD></B>

---

## 事件驱动

事件驱动的<B>核心逻辑</B>为：<B><VT>外部实体向程序的<GN>事件循环event loop</GN>或<GN>运行循环run loop</GN>生成事件</VT></B>

### 异步IO库的简单实现

如果想使用<B>异步IO库</B>，我们可能有如下代码：
...
对于输入字符串，输出字符串的简单情况，最简单的方式必然是<B>同步实现</B>：
...
- <B>同步</B>---堵塞式，会导致<VT>线程挂起无法处理其它操作</VT>
- <B>异步</B>---递归会导致<VT>多层嵌套，代码不清晰</VT>

所以可以使用<B>异步模拟同步</B>，获得更好的效果：
