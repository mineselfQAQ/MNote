<center><B><BBBG>C#反射</BBBG></B></center>
...
反射是一种比较常见的功能，简单来说就是：
<B><VT>程序运行时，获取并操作"类型本身"的能力</VT></B>

<B>重要：<DRD>反射虽然方便，但是很慢</DRD></B>
...
Assembly即<B><GN>程序集(dll/exe)</GN></B>，某些情况下需要从<B>Assembly</B>开始<B>获取</B>，操作如下：
...
获取到Assembly对象后，最常用的就是<B>获取Assembly下的类型</B>：

- `asm.GetTypes()`
- `asm.GetType()`　　<VT>需要传入FullName(命名空间+类名)</VT>

<B><BL>问题：为什么有时获取不到Type</BL></B>
...
Activator即<B><GN>激活器</GN></B>，从名字上来看就是用来<B>创建</B>的
...
<B>注意：</B>
<B><DRD>`(Type type, bool nonPublic)`版本指定了访问性，除复杂版本(带BindingFlags)，都等价于寻找public版本</DRD></B>
...
对于获取对象，非常常见的一种参数就是<B>BindingFlags</B>，从名字上来看，其含义为<B>绑定标识</B>，其实各类控制开关
<B>常用的</B>如下所示：
...
- 无参情况<B>默认</B>为`BindingFlags.Public | BindingFlags.Instance`
- 4种访问修饰符<B>不是互斥的</B>
- Public包含public
...
反射中能获取到各种对象，每种对象都对应着一种<B>XXXInfo</B>
...
可以看到MemberInfo是<B>最基类</B>，大部分其它的Info都会继承于它，因为说到底是某种对象
值得注意的是<B><VT>Type也是继承于MemberInfo的</VT></B>

既然是最基类，那么其中必然具有一些<B>通用内容</B>：
...
Method即<B>方法</B>，Constructor即<B>构造方法</B>，本质上两者都是方法，所以它们有一基类MethodBase
由于是方法，所以有一些<B>共有内容</B>：
...
但也有其它情况，也就是<B>索引器</B>：`public string this[int index] { get; set; }`，其获取方法为：
...
Attribute即<B>特性</B>，也就是在内容前的`[XXX]`
<B>内容</B>有：
...
<B>对于Type情况</B>，分为3种情况： <VT>以`class XXX<T> {}`为例</VT>
...
<B><BL>问题：为什么字段之类的没有，明明有`List<T>`这种</BL></B>
