<center><B><BBBG>LoxodonFramework分析第二版</BBBG></B></center>
...
LoxodonFramework是一套<B><VT>基于Unity</VT></B>的<B>UI框架</B>
阅读LoxodonFramework源码后，可以直观地感受到它的<B>特性：<VT>代码结构商业化</VT></B>
所以说该框架是非常<B>值得学习</B>的

首先先看一下其<B>功能方向</B>，也就是它的文件夹：
![](Pic/Loxodon1.png)
整体来说这些内容是相辅相成的，主要目标还是为了构建UI框架
在上述文件夹中，我们能发现<B>ViewModels/Views</B>，这也说明了该框架是<B><VT>基于MVVM</VT></B>的
...
可以看到这些内容与框架内容是<B>强关联</B>的,可以说大部分情况下是由框架提供的这些能力
...
<B><YL>这里用LoginWindow来展示：</YL></B>
先看一下两者的<B>声明</B>：
`public class LoginWindow : Window`
`public class LoginViewModel : ViewModelBase`

观察View脚本<B>LoginWindow.cs</B>，可以说干了两件事：
...
而ViewModel脚本<B>LoginViewModel.cs</B>当然会与其配对：
...
在这里的话就是<B>Account.cs</B>，一个<B>实体</B>：
...
由于它具有和ViewModelBase一样的<B>基类ObservableObject</B>，所以如`Username`属性是非常相似的，但Account从<B>功能</B>上还是<B>不太一样</B>的：
它牵扯到两个类：<B>AccountRepository/AccountService</B>

<B>AccountRepository</B>是<B><VT>领域层的一部分，是Account的存储地(集合)</VT></B>
这是<B><VT>一种拆分</VT></B>，本来在实体Account中可以添加如`Get()`/`Save()`等函数(一个类即可完成所有)，仓储将函数剥离出来，使用起来必然是<B>更有秩序</B>的
...
<B>AccountService</B>则<B><VT>注入了IAccountRepository</VT></B>提供了相关功能，有：
...
<B>Provider</B>即提供者，能够<B><VT>提供数据</VT></B>
<B>核心点：<VT>提供能力</VT></B>
...
<B><DRD>注意：</DRD></B>
<B><DRD>Service属于服务层，而Locator/Provider属于基础层，这是因为本质上来说服务层是业务相关，而基础层是业务无关的
Locator/Provider按道理来说不可能是服务层，因为即使它的功能是Locator/Provider但命名依旧应该为Service</DRD></B>
<B><VT>Tip：Locator/Provider虽然看起来业务相关，但应该称之为整体业务相关，可能就是针对这整个项目编写的特化代码，但不会使用到任何内部数据</VT></B>
...
框架本身就是<B>UI框架</B>，可以说所有创建的内容都是为了实现好的UI效果而创建的
前面也提到过，该框架使用的是<B><GN>MVVM</GN></B>，其中<B>最重要的就是绑定</B>
简单分类的话可以这么分：
...
无论是框架还是绑定，都将用到许多<B>组件辅助类</B>之类的内容，这里先都分析一下
内容比较多，可以在用到后再进行查阅
<B>简单来说，可以分为以下几种：</B>
...
`Debug.Log()`可以说是我们在Unity中最常用的一个API了，<B>功能</B>就是<VT><B>输出信息</VT></B>，该框架对其进行了一定的<B>扩展</B>
...
显然Manager仅为组织者，具体还是需要<B>看Factory</B>
...
<B><VT>ObjectPool在框架中并无使用，但是作者还是提供了该功能</VT></B>
一共有<B>2种形态</B>：
...
对于这些框架来说，命令模式最大的<B>好处</B>应该是<B><VT>可拆分</VT></B>，但是在该框架下<B>并非如此</B>

所有类型的Command都基于<B>CommandBase</B>，<B>声明</B>为：
...
<B><GN>SimpleCommand</GN></B>
SimpleCommand是几种Command中最简单的，先看<B>构造</B>：
...
而对于SimpleCommand来说，<B>不存在Enable属性变更导致的事件触发</B>

<B><GN>RelayCommand</GN></B>
Relay即转发，从名字上不太好理解，但我们看一下<B>构造函数</B>就清楚了：
...
<B>Preferences</B>与<B>Configuration</B>两者都是有关存储的内容
当然它们有着<B>不同的侧重点</B>：
...
#### Preferences
Preferences是<B>ApplicationContext的重要组成部分</B>，有：
...
- <B>获取</B>
  获取其实和创建是合二为一的，与大部分类相同，这里同样使用的是<B>工厂模式</B>，方法如下：
  <BR>
...
<B><GN>PlayerPrefsPreferences</GN></B>
...
所以<B>`Load()`</B>所做的就是：
<B><VT>获取PlayerPrefs所存储的数据集，并拆解成List形式(`keys`)</VT></B>

<B>存取</B>是通过<B>`SetObject()`/`GetObject()`</B>完成的：
...
<B><GN>BinaryFilePreferences</GN></B>
...
<B><GN>ConfigurationBase</GN></B>
四种Configuration的基，简单分析一下<B>核心内容</B>：
...
对于这些实现的内容，我们可能会比较好奇的是`Subset()`，这显然<B>与SubsetConfiguration有关</B>
...
我们可以得知：
<B><VT>MemoryConfiguration核心是加载dic，并对其进行增删查操作</VT></B>
...
以上是基础的两个Configuration，接下来看一下<B>复合型</B>的：
...
<B>parent</B>与其名中的<B>SubSet子集</B>对应：
<B><VT>SubsetConfiguration是某Configuration的子集</VT></B>
...
Subset意味着子集，而Composite意味着<B>组合</B>，即<B><VT>由多个Configuration组成的Configuration</VT></B>
其<B>构造函数</B>说明了一切：
...
可以说这是一种<B>具有保底机制的Configuration</B>：
<B><VT>依次查看Configuration有没有对应key，最终保底一个memoryConfiguration</VT></B>
<B>也就是：</B>
...
<B><GN>ObservablesList/ObservableDictionary</GN></B>
...
<B><VT>Tip：由于脱离了C#MVVM，所以自动绑定的机制不再可用，但可自行实现</VT></B>
<B>接口实现</B>本身很简单：
...
那么<B>重中之重</B>其实是<B>PropertyChanged事件的添加</B>，但这属于是业务层面的内容了

<B><GN>INotifyCollectionChanged</GN></B>同理，但处理的是<B>集合级别</B>的
但是有一点<B>需要注意</B>：
...
ObservableProperty本质上和ObservablesList/ObservableDictionary可以<B>归为一类</B>，但是有一点<B>明显不同</B>：
<B><VT>ObservableProperty不继承C#接口而是自定义了接口</VT></B>
其<B>声明与接口</B>如下：
...
由此我们能更加明白<B>Object的含义</B>：
<B><VT>ObservableObject本身就是一种需要观察者功能的集合，是其中的属性需要进行`Set()`操作</VT></B>

<B><BL>问题：`[CallerMemberName]`是什么</BL></B>
<BL><B><GN>[CallerMemberName]</GN></B>，这是一个<B>编译期特性</B>，可以<B><VT>自动传入属性名</VT></B>
所以对于上述例子，就相当于是：`this.Set(ref this.username, value, "Username")`
那么这里的<B>含义</B>就是：<B><VT>先将`username`设置为value，然后通过该string转换为PropertyChangedEventArgs后进行propertyChanged回调</VT></B></BL>

<BR>

#### Interactivity

Interactivity即<B>互动性</B>，对于该组内容需要先<B><YL>举一个例子</YL></B>明确一下用法：
...
- <B><VT>Interactivity是一系列用于绑定情况(Button)的事件触发机制</VT></B>
- <B>涉及项</B>有如下几种：
...
<B>Request</B>
Request都继承于<B>接口IInteractionRequest</B>，如下所示：
...
- <B>InteractionRequest</B>
- <B>AsyncInteractionRequest</B>：异步形态
...
<B>Action</B>
在上述例子中，Action并没有任何类，而是<B><VT>通过一个形参为`(object sender, InteractionEventArgs args)`形式的函数</VT></B>完成的，但作者提供了一些<B>预制Action</B>，有：
...
Messaging即消息，涉及类不多，<B>核心</B>为<B><GN>Messenger</GN></B>，在项目中我们可能会看见：
...
<B>可以发现：</B>
<B><VT>Subject依旧是一层封装，由`Messenger.Subscribe()`会保存Subscription在内部，最终还是通过`Subscription.Publish()`完成发布操作</VT></B>

<B><GN>Subscription</GN></B>是<B>Publish的本质</B>，如下所示：
...
三者其实分别是<B>设计模式的不同体现</B>：

- Observables：观察者模式，用于<B><VT>数据通知</VT></B>
- Interactivity：观察者模式，用于<B><VT>沟通V与VM</VT></B>
- Messaging：发布订阅模式，用于<B><VT>解耦通信</VT></B>

<B><BL>问题：观察者模式与发布订阅模式的区别</BL></B>
<BL><B>核心点</B>在于：</BL>
<B><VT>观察者模式下观察者与被观察者是耦合的---观察者订阅被观察者事件
发布订阅模式下发布者与订阅者完全不耦合---发布者与订阅者不知道彼此的存在，完全通过中间件通信</VT></B>
...
<B>Observables与Interactivity的区别在于：</B>
<B><VT>Observables是传统的观察者模式，而Interactivity是针对于MVVM的观察者模式(将Request与Action拆解在VM与V中，由绑定构成观察者模式)</VT></B>

### Asynchronous

Async我们肯定很熟悉，即<B>异步</B>，但它的全称就是Asynchronous(adj.)
在Unity中我们可能完全用不到异步，因为使用携程即可代替，但是这不意味着异步是没有用的

#### AsyncResult

对于异步部分，有一个<B>基础核心类</B>，为<B><GN>AsyncResult</GN></B>：
...
- <B>IAsyncResult接口</B>显然是AsyncResult的基，可以看到最重要的2数据：
  - Result：异步完成结果
  - IsDone/IsCancelled：状态(完成/取消)
- <B>IPromise</B>接口从名字上指的是承诺，即<B><VT>承诺完成并给出结果(无论成功还是失败)</VT></B>，这是<B>JavaScript异步中的一个概念</B>

<B><BL>问题：IAsyncResult与IPromise的区别</BL></B>
...
说到底一切都是为了在异步情况下用一个实例存储Result并记录状态，那么<B>Set</B>显然是其中<B>最重要</B>的
<B>函数`SetXXX()`</B>
...
<B>这里的`Monitor.PulseAll(_lock)`非常关键：</B>
该函数是有着一对<B>匹配项</B>的：
...
<B>函数`WaitForDone()`</B>
从名字上来看，这是一个<B><VT>用于等待执行完成</VT></B>的函数，其<B>用法</B>如下：
...
<B>本质：<VT>Unity的`UnityEngine.WaitWhile()`</VT></B>
这个函数我们可能不熟悉，但是我们一定见过<B>WaitForSeconds类</B>，其实是一样的

<BR>

除了以上两种函数外，仅剩2种<B>比较特殊的函数</B>了，即`Callbackable()`/`Synchronized()`
...
由此我们可以得知：
<B><VT>Callbackable是一个回调缓存地，这是因为异步的不确定性而添加的，我们不清楚当前的完成状态</VT></B>

当然，AsyncResult只是最基础的一种异步Result，还有一些<B>派生</B>于它的<B>类</B>：
...
<B>目的</B>为：当UpdateProgress()时，<B><VT>根据注册回调完成相应操作</VT></B>，<B><YL>可能有：</YL></B>
...
看过了<B>ProgressResult</B>与<B>ImmutableAsyncResult</B>，该类就很明显了，即<B><VT>两者结合</VT></B>

<B><GN>CoroutineResult</GN></B>　　<VT>CoroutineProgressResult同理</VT>
该类严格来说并非基础的AsyncResult，因为它是在<B>Loxodon.Framework.Execution命名空间</B>下的
...
但是<B>几乎所有的Task</B>都被标记为`[Obsolete]`即<B>不再使用</B>

<B>唯一被保留</B>的是<B><GN>CoroutineTask</GN></B>：
...
这其实是一种对<B>await关键字</B>的扩充，所以也就需要进行编译限定了
可能的实现如下：
...
Execution即执行，代码本身就会执行，那么这里的执行指的就会是<B>特殊形式的执行</B>

#### Executors

其中的<B>核心类</B>为<B><GN>Executors</GN></B>：
浏览代码，会发现内部有大量的<B>静态函数</B>，事实上它们都是<B><VT>来自各个分支Executor的</VT></B>：
...
<B>即：<VT>Executors是一个集中类，汇聚了所有执行操作</VT></B>

<B><BL>问题：明明创建了如MainLoopExecutor的分支类，但实际上几乎没有使用，还是在使用Executors</BL></B>
...
显然这是一个<B>初始化操作</B>，其实<B><VT>必定会执行一次</VT></B>：
...
可以看到的是当场景加载前一定会执行一次Create()，AbstractExecutor只是起了<B>保护作用</B>
<B>`Create()`中的几个变量都是关键：</B>
...
<B><BL>问题：为什么MainThreadExecutor被内置在其中</BL></B>
...
<B><DRD>注意：这是Executors.MainThreadExecutor而非MainLoopExecutor，别看错了</DRD></B>

#### 分支Executor

<B><GN>MainThreadExecutor</GN></B>
...
<B>具体如下：</B>
该类提供了2个公开函数供Executors使用，即`Execute()`/`Stop()`，一个<B>开始</B>一个<B>结束</B>
简单来说：

- `Execute()`：加入pendingQueue
- `Stop()`：加入stopingQueue，或移出pendingQueue<VT>(对于IEnumerator来说，可能还未开始)</VT>

具体的执行由<B>MonoBehaviour的生命周期</B>控制：
...
简单来说，<B>会对相应队列内的所有内容(IEnumerator或Coroutine)执行所对应的操作</B>：
...
<B>泛型形式</B>区别不大，为<B>waitForExecution为true</B>情况的<B><VT>一种变体</VT></B>
更核心的是一般形式，<B>泛型形式</B>仅是一种<B><VT>特例</VT></B>
...
可以看到这里与其它Executor<B>有所区别</B>：
<B><VT>ThreadExecutor并非直接调用Executors的静态函数，而是将action包装了一层</VT></B>
...
以<B>最简单形式</B>为例： `public virtual Asynchronous.IAsyncResult RunOnCoroutine(IEnumerator routine)`
...
可以看到这里用到的是<B>CoroutineResult</B>，即前面Result分析中没有详细分析的一种，这是一种<B><VT>专用特例</VT></B>
<B><GN>CoroutineResult</GN></B>是AsyncResult的扩展，我们需要记住的是：
<B>CoroutineResult核心扩展了`AddCoroutine()`功能</B>
...
<B><GN>ThreadScheduledExecutor/CoroutineScheduledExecutor</GN></B>
这两种就是<B>Scheduled版本</B>，简单来说是<B><VT>周期执行版本</VT></B>
作者提供了例子：
...
总的来说相似程度极高，但是它们还是<B>有区别</B>的：

<B>MainThread</B>相对来说和其它几种不太一样，它更像是一种<B><VT>底层类</VT></B>
由于它<VT>基于MonoBehaviour</VT>，即<B>主线程</B>
之所以称其为底层类，是因为该类是<B>为了其它几种类</B>的需求编写的：
...
<B>MainLoop/Thread/Coroutine</B>看起来多多少少有些相似，但是区别是存在的
从<B>输入</B>上来看：
...
<B>所以：</B>
<B><VT>几种形式最终的目的是一致的，只是使用的方法有所不同，根据实际情况选择即可</VT></B>
...
<B><GN>Context</GN></B>
Context为上下文基类，<B>声明</B>如下：
`public class Context : IDisposable`
内容实际上很简单，<B>简单总结</B>如下：
...
<B>特点：<VT>每一层都各自隔离，子可以找到父的内容</VT></B>

<B><GN>ApplicationContext</GN></B>
...
<B><BL>问题：为什么Context这里有了Preferences，还提供了属性存取功能</BL></B>
<BL>这里具有了与Preferences不太相同的作用域机制，所以可能会是有用的</BL>

<B><GN>PlayerContext</GN></B>
PlayerContext仅有一个新属性Username，这代表着<B>子层级名字</B>
...
根据Context的内容，默认会创建<B>ServiceContainer</B>
<B><GN>ServiceContainer</GN></B>的<B>声明</B>如下：
...
最终，注册内容被<B>放入字典</B>中供查询使用(通过type/name可查询)
同时还有<B>泛型注册方式</B>：
...
以上内容都是框架提供的边边角角，我们可以简单地认为这些都是<B>功能</B>
而关键的<B>MVVM</B>及其<B>绑定</B>还没有提及
...
M在三者中是比较简单的一个，即<B>数据Model</B>
作者在文档中提到了，其<B>建议</B>是使用<B><GN>贫血模型</GN></B>
上述例子中有：

- 领域层
  - Account：<B>实体</B>
  - AccountRepository：<B>仓储</B>
- 应用层
  - AccountService：<B>服务</B>

补充概念：<B>值对象</B>---<VT>不具有唯一标识且属性不可变的数据</VT>
<B>简单来说：</B>
<VT>Account是数据本身，AccountRepository加以包装并提供方法，AccountService向外界提供服务</VT>

<B>实体</B>
我们所认为的<B>M</B>指的一般就是<B>实体</B>，也就是数据，也是在视图上需要显示的内容
<B><YL>以Account为例：</YL></B>
...
可以看到MonoBehaviour是Unity的类，而<B><GN>UIBehaviour</GN></B>其实是<B>UGUI的扩展类</B>，之后又逐步扩充为Window

<B><GN>UIView</GN></B>
第一个扩展类为UIView，显然<B>接口IUIView</B>指示了扩展内容：
...
除此以外，<B>EnterAnimation/ExitAnimation</B>则是特殊的，<B>在该类中并未赋值</B>：
...
<B>总结：</B>
<B><VT>UIView就是一个基于UIBehaviour的很简单的基类，记录了一些gameObject自带的信息</VT></B>

<B><GN>WindowView</GN></B>
同样的，<B>接口IWindowView</B>指示了扩展内容：
...
看起来和UIView的EnterAnimation/ExitAnimation非常相似，同样<B>在该类中并未赋值</B>
...
<B>重点：<VT>Views即时获取，方法为在其子gameObject中寻找带有IUIView组件的</VT></B>
除此以外，具有如下<B>View操作函数</B>：
...
该类具有<B>2个接口IWindow/IManageable</B>，但其实是嵌套接口：
...
- `Show()`/`Hide()`/`Dismiss()`：<B>WindowManager扩展</B>
- `Activate()`/`Passivate()`/`DoShow()`/`DoHide()`/`DoDismiss()`：<B>原生</B>
  
总的来说它们都是<B><VT>动画组成的重要函数</VT></B>

<B><DRD>要注意：看似Window是对应着一个界面，但并不如此</DRD></B>
...
<B><GN>WindowManager</GN></B>
WindowManager是Manager核心，先看<B>声明</B>：
...
这里大部分内容看名字也能大概看出实现方式，其中值得注意的应该就是<B>动画操作部分</B>了，这是<B><VT>Window中同名函数实现的本体</VT></B>

值得注意的是<B>Activated</B>：
...
观察示例项目即可发现，<B>Canvas下存在GlobalWindowManager组件</B>
...
<B>GlobalWindowManagerBase</B>同样也是<B>扩展</B>：
...
其实就是一个<B>自动管理扩展</B>：<B><VT>开始时注册进`Root`，结束时移除</VT></B>

<B><GN>WindowContainer</GN></B>
有Global就自然有Local，但是这里的<B>WindowContainer</B>不太一样，先看声明：
...
<B>值得注意的是`container.Create()`以及override的`OnCreate()`：</B>
...
<B>这样我们应该就能理解了：</B>
<B><VT>WindowContainer是一个自带WindowManager的Window</VT></B>
浏览代码就能发现，<B><VT>IWindowManager接口的实现都是通过直接包装</VT></B>完成的：
`public IWindow Current { get { return localWindowManager.Current; } }`
反而<B>override有来自Window的2个函数</B>：`Activate()`/`Passivate()`

<B><BL>问题：为什么gameObject不在根节点上，而在Canvas下</BL></B>
...
在上述Window/WindowManager中，值得注意的是<B>动画部分</B>，也就是如`Show()`等操作
<B><YL>回顾Launcher.cs：</YL></B>
...
<B><GN>Transition</GN></B>
上述代码也可以看出动画与<B>ITransiton</B>密不可分，其内容如下：
...
其<B>抽象派生类Transition</B>才是关键
内容有很多，但绝大多数都是<B>回调相关</B>：
...
可以看到这是一个<B><VT>携程操作</VT></B>，执行内容主要为`DoTransition()`，<B>需要子类实现</B>

<BR>

<B><GN>Animation</GN></B>
除了上述Transition，Window中还有<B>IAnimation接口</B>的存在：
...
也就是很简单的<B>一个操作以及两个附加连点</B>
其<B>抽象基类</B>为<B><GN>UIAnimation</GN></B>，内容也很简单：
...
<B><VT>Window中未赋值的Animation都是由它生成，只需在gameObject中添加一个即可</VT></B>
就像这样：
...
<B><BL>问题：Transition与Animation的区别</BL></B>
...
<B>详细来说：</B>
<B><VT>Passivate()/Activate()与用于控制`Activated`，DoHide()/DoShow()用于控制`Visiblity`
<DRD>存在先后关系</DRD>：
DoShow()后界面显示，`Visiblity`为true，可进行`Activate()`操作，想关闭界面时，进行`Passivate()`后可再进行`DoHide()`
但是反过来，直接进行`Passivate()`根本不成立，界面不显示不允许进行，直接进行`DoHide()`虽然可行，但之后不能再进行`Passivate()`操作了</VT></B>
<B>更本质一点来说：</B>
<B><VT>DoHide()/DoShow()是显隐操作，Passivate()/Activate()是之后的回调触发器</VT></B>
<B><DRD>注意：需要区别Window与WindowManager的Activated，Window与WindowManager两者层级不同，WindowManager会比Window高一级</DRD></B>
...
<B>核心</B>为`this.WindowManager.Remove(this)`/`GameObject.Destroy(this.gameObject)`，是<B><VT>完整清理操作</VT></B>
...
<B>更重要的</B>是接下来的这句：
`GetTransitionExecutor().Execute(transition)`
`GetTransitionExecutor()`是一个<B>单例获取器</B>，用于获取<B>BlockingCoroutineTransitionExecutor</B>
<B><GN>BlockingCoroutineTransitionExecutor</GN></B>从名字上可以大概了解到：这是一个<B><VT>堵塞式携程Transition执行器</VT></B>
该执行器仅有2个操作：<B>Execute()/Shutdown()</B>，即<VT>需要执行Transition时`Execute()`，需要强行中断时`Shutdown()`</VT>
...
显然，这里的<B>核心</B>为`Executors.RunOnCoroutine(this.DoTask())`：
...
显然，这里的<B>核心</B>为`Executors.RunOnCoroutine(transition.TransitionTask())`
终于，这是在Transition提及过的函数，本质上就是<B>DoTransition()</B>，即<B>ShowTransition实现的那个</B>

<B><VT>Show()是如此，Hide()/Dismiss()也是同理</VT></B>
<B>区别</B>在于：
...
<B>1.两组动画操作Show()/Hide()/Dismiss()与Activate()/Passivate()/DoShow()/DoHide()/DoDismiss()的区别</B>
...
<B>2.Activate()/Passivate()与DoShow()/DoHide()/DoDismiss()的区别</B>
...
- Show()的操作`DoTransition()`在<B>ShowTransition</B>中实现，会由单例的<B>BlockingCoroutineTransitionExecutor</B>执行该transition
...
- `DoTransition()`是transition的核心，也就是过渡内容，无非就是`Activate()`/`Passivate()`/`DoShow()`/`DoHide()`/`DoDismiss()`进行开关操作
  - <B>Animation</B>是这些函数的具体动画操作，这是通过挂载AlphaAnimation完成的

<BR>

##### VM
与Window同样重要的也就是VM，在这里就是<B><GN>ViewModelBase</GN></B>，其<B>声明</B>为：
`public abstract class ViewModelBase : ObservableObject, IViewModel`
ViewModelBase<B><VT>继承于ObservableObject</VT></B>，这意味着它可以使用`Set()`进行设置并回调
而<B>IViewModel接口</B>非常简单，是一个单纯的IDisposable：
`public interface IViewModel : IDisposable {}`

<B>ViewModelBase</B>本身其实也很简单，本质上<B><VT>仅扩展了`Set()`</VT></B>，以下是其中一种：
...
<B>这么看的话，VM并无任何特殊之处，和M甚至是类似的，但不可能只是这样</B>

<BR>

#### 绑定

通过上述MVVM的分析，虽然对每个类的本质稍微了解了一些，但是<B>如何串联起来</B>才是关键
...
<YL>可以看到<B>V与VM是相辅相成的</B>，就拿上面例子的一条来看：
`bindingSet.Bind(this.username).For(v => v.text, v => v.onEndEdit).To(vm => vm.Username).TwoWay();`
...
从上文中可以看出绑定的工具为<B>BindingSet</B>，进行了一系列`Bind()`操作，最终`Build()`绑定完成，这就是表面上绑定的一切

在进行绑定流程分析前，先对一些细节方面的类进行分析

##### Path

<B>Path</B>即路径，实际上有很多需要调用到的地方
...
虽然可以传入<B>string</B>，但最常用的还是<B>Lambda表达式</B>
...
可以看到这里取了Lambda表达式的<B>Body部分</B>进行解析，也就是<VT>取了`=>`后面的内容</VT>
情况很多具体如下：
...
首先会创建一个<B>Path</B>，根据Body的情况深入执行(提前分类了而已)，即调用<B>底层`Parse()`</B>进行path填充：
...
<B><GN>PathToken</GN></B>是<B>Path的扩展</B>，<B><VT>具有index指针，可通过`Current`获取当前Path，也可通过`NextToken()`生成下一步的PathToken</VT></B>
...
以上都在通过Lambda表达式进行解析，对于<B>string</B>有特殊的一项：
...
以上为Path的核心部分内容，此外还存在一些<B>功能类</B>：
...
该操作是一个<B>扩展函数</B>，被写在<B><GN>ProxyTypeExtentions</GN></B>中，函数如下：
...
除了Type，还支持其它<B>变体</B>：EventInfo/FieldInfo/PropertyInfo/MethodInfo，
...
简单描述一下，就是：
<B><VT>将Unity内置的IProxyMemberInfo信息提前注册完毕，在后续使用中方便使用(如`GetField()`可快速取出，`GetValue()`可通过传入getter获取，`SetValue()`同理)</VT></B>
...
通过上述的例子，很明确地了解到了<B>BindingSet承担了绑定的职责</B>，那么就沿着BindingSet的流程看一看

<B><GN>BindingSet</GN></B>
在看类之前，先看一下获取：
`BindingSet<LoginWindow, LoginViewModel> bindingSet = this.CreateBindingSet<LoginWindow, LoginViewModel>();`
我们可能会疑惑该类(LoginWindow)并没有`CreateBindingSet()`函数的存在，这其实是因为被写在了<B>BehaviourBindingExtension.cs</B>中了：
...
Window组件上的gameObject上自动添加一个<B>BindingContextLifecycle组件</B>，组件内部会创建一个<B>BindingContext</B>

<B><GN>BindingContextLifecycle</GN></B>
...
<B><GN>BindingContext</GN></B>
该类是一个<B><VT>数据绑定信息管理类</VT></B>
从<B>构造函数</B>可以看出一些端倪，最完整版的如下：
...
对于该类，<B>还有1点需要注意</B>：
<B>DataContext属性的set方法具有2个回调相关内容</B>：<B><VT>触发自身的`DataContextChanged`以及对所有存储的IBinding的DataContext进行修改(这说明DataContext本质上只有1个)</VT></B>

<BR>

我们使用的BindingSet，有很多泛型变体，但它们都有一个基类，即<B>BindingSetBase</B>
<B><GN>BindingSetBase</GN></B>是一个抽象类，<B>声明</B>如下：
`public abstract class BindingSetBase : IBindingBuilder`
<B>接口</B>内容仅有`Build()`一个函数
...
context，也就是IBindingContext，这很熟悉，是前面提到的<B>数据绑定信息管理类</B>
...
在此之前，先把<B>BindingBuilder的连点</B>了解一下，这里<B><YL>随意取一个函数作为例子</YL></B>：
...
可以预料到的，<B><VT>更多的函数都是在为description填充数据</VT></B>，<B>总结</B>一下：
...
现在我们需要了解的第一目标为<B>绑定所需的参数</B>

<B>回忆一下BindingContext的完整构造：</B>
`public BindingContext(object owner, IBinder binder, object dataContext, IDictionary<object, IEnumerable<BindingDescription>> firstBindings)`
可查询到的BindingContext创建方式如下：  <VT>出自BehaviourBindingExtension.cs</VT>
`bindingContext = new BindingContext(behaviour, Binder);`
即仅传入owner/binder的版本，借此我们也能了解到它们的含义：

- <B>owner</B>：拥有者，也就是创建者(即behaviour，是它创建的BindingContext)
- <B>Binder</B>：绑定工具，也就是上述提到的注册的StandardBinder

<BR>

<B>再看`Binder.Bind()`的构造：</B>
IBinder接口有：
...
- <B>bindingContext</B>：`this`，传入自身，本质上就是<VT>一种提供额外信息的方式</VT>，可以说有了它什么都能获取
- <B>source</B>：`this.DataContext`，是自身的一个属性，结合target可知，既然是source那么就意味着<VT>DataContext就应该是ViewModel</VT>
- <B>target</B>：`target`，追溯回`this.CreateBindingSet()`，可知指的是Window，同时前面也有所提及，<VT>target即目标即Window即view</VT>
- <B>bindingDescription</B>：`description`，也是前面提到过的连点信息填充处，显然<VT>存储了一些绑定个体信息</VT>

<BR>

<B><BL>问题：dataContext在BindingContext并未传入，`Add()`时会存在吗</BL></B>
...
<B><BL>问题：BindingDescription是什么，怎么传入的</BL></B>
...
通过前面的讲述应该已经能够了解到了：<B>`Binder.Bind()`是绑定的核心</B>
接下来就围绕着该函数进行分析

Binder使用的是<B>StandardBinder</B>
...
先看最大的<B>BindingFactory</B>：
<B><GN>BindingFactory</GN></B><B>声明</B>如下：
`public class BindingFactory : IBindingFactory`
<B>接口</B>提供一个函数：`Create()`，这正是绑定工厂应该做的
...
外部传入的这些都是非常熟悉的内容了，<B>2个工厂</B>值得我们注意

先看<B>SourceProxyFactory</B>：
...
<B>ISourceProxyFactory接口</B>提供了`CreateProxy()`，<B>ISourceProxyFactoryRegistry接口</B>提供了`Register()`/`Unregister()`
...
ObjectSourceProxyFactory与TargetProxyFactory整体也是<B>同理</B>的：
<B><VT>SourceProxyFactory/TargetProxyFactory以及内部的ObjectSourceProxyFactory都是排序工厂，会按优先级依次尝试创建Proxy</VT></B>

<BR>

<B>细看SourceProxyFactory的子Factory：</B>
有3个，优先级从低到高有：LiteralSourceProxyFactory/ExpressionSourceProxyFactory/ObjectSourceProxyFactory

它们三个类都继承于同一类<B>TypedSourceProxyFactory</B>
...
<B><DRD>注意：这里的是ObjectSourceDescription而非BindingDescription</DRD></B>
...
<B><GN>ObjectSourceProxyFactory</GN></B>
ObjectSourceProxyFactory具有最高优先级，也就意味着会是<B>首选</B>的那个
其<B>声明</B>如下：
...
两<B>接口</B>非常类似于SourceProxyFactory的，因优先级选择而存在
...
检查这些Proxy的基，会发现基应该为<B>SourceProxyBase</B>
<B><GN>SourceProxyBase</GN></B>是一个抽象类，<B>声明</B>如下：
`public abstract class SourceProxyBase : BindingProxyBase, ISourceProxy`
BindingProxyBase没什么特别的，只是提供了IDisposable功能，
同时其本身的<B>ISourceProxy接口</B>也简单：
...
显然<B>Source</B>是其中的核心，函数名SourceProxyBase也说明了这一点，仅为<B><VT>Source的代理</VT></B>

该基类有一个<B>扩展</B><B><GN>NotifiableSourceProxyBase</GN></B>，<B>声明</B>如下：
`public abstract class NotifiableSourceProxyBase : SourceProxyBase, INotifiable`
显然是<B><VT>通知功能</VT></B>，<B>INotifiable接口</B>提供了事件：`event EventHandler ValueChanged`

对于派生类，可能存在两种接口<B>IObtainable</B>/<B>IModifiable</B>，即<B><VT>修改与设置</VT></B>：
...
<B>接下来就看一下具体的这些Proxy：</B>
`path.Count <= 0`即非正常情况，就比如说`vm => 10`(正常来说肯定得是vm的一个内容，如`vm => vm.Username`)，此时为<B>LiteralSourceProxy</B>
<B><GN>LiteralSourceProxy</GN></B><B>声明</B>如下：
...
`path.Count == 1`是最常用的情况，就比如说`vm => vm.Username`，此时path中只有一个Node，此时使用的是额外的<B>UniversalNodeProxyFactory</B>
...
由于path的数量只有1，所以token自然取出的<B>是唯一的Node</B>，具体会根据`node.IsStatic`决定Proxy
<B><BL>问题：IsStatic取决于什么</BL></B>
...
`path.Count >= 2`是一种更深层的情况，就比如说`vm => vm.User.Name`，相比=1情况深度更深了，但也仅仅只是多几层递归而已，此时为<B>ChainedObjectSourceProxy</B>
...
<B><GN>ProxyEntry</GN></B>即<B>Proxy扩展</B>，存储了ISourceProxy/PathToken，也就是Proxy以及其对应位置
`Bind()`是关键：
...
<B><BL>问题：为什么那么复杂</BL></B>
<BL>对于动态内容，复杂是自然的，就以`vm => vm.User.Name`为例，关键在于：<B><VT>User的改变会导致Name的改变</VT></B>，这也是代码中`Rebind()`的原因</BL>

<BR>

<B><GN>ExpressionSourceProxyFactory</GN>/<GN>LiteralSourceProxyFactory</GN></B>
对于这两种优先级并非最高的Factory，本质上只是一种<B>callback</B>
只需要知道：<B><VT>SourceProxyFactory是用来生成SourceProxy的</VT></B>

<BR>

再看<B>TargetProxyFactory</B>：
...
所以这里选择看一下<B>UnityTargetProxyFactory</B>：
...
事实上，三种Proxy的基都为<B>TargetProxyBase</B>，<B>声明</B>为：
`public abstract class TargetProxyBase : BindingProxyBase, ITargetProxy`
这与SourceProxyBase是极其类似的，其拥有参数在<B>ITargetProxy接口</B>可知：
...
相比Source的多了一个<B>DefaultMode</B>，这也变相说明了<B><VT>BindingBuilder中执行的类似`TwoWay()`操作是为了Target执行的</VT></B>

<B><GN>UnityEventProxy</GN></B>
<B>继承链</B>如下：
...
对于这种情况，可以了解到它具有<B>IModifiable</B>功能
<B>初始绑定</B>是最关键的：
...
<B><GN>UnityPropertyProxy</GN>/<GN>UnityFieldProxy</GN></B>
<B>继承链</B>如下：
...
功能方面它们都具有<B>IModifiable/IObtainable/INotifiable</B>功能

<BR>

以上就大致分析完SourceProxyFactory/TargetProxyFactory了，简单来说：
<B><VT>两者都是选择工厂，最终目的都是创建一个Proxy，这通过的是前期`For()`/`To()`以及其它函数得到的信息以及额外信息组合得来</VT></B>
回忆一下，最终的创建函数为：
...
也就是说<B><GN>Binding</GN></B>是绑定完成返回的类，其<B>声明</B>如下：
`public class Binding : AbstractBinding`
`public abstract class AbstractBinding : IBinding`
其基类<B>AbstractBinding</B>并没有太多内容，主要就是简单实现了一下<B>IBinding接口</B>：
...
可以看到BindingMode中Target与Source的条件竟然<B>不是互补的</B>，观察可知：

- <B>OneWay</B>：Source->Target，Source触发
- <B>TwoWay</B>：Source<->Target
- <B>OneWayToSource</B>：Target->Source，Target触发
- <B>OneTime</B>：Source->Target，Source触发，仅在首次触发(`UpdateDataOnBind()`情况)

由此可见从Source到Target是默认流向

<B>INotifiable接口</B>值得我们注意：
...
那么具体如何更新，这就取决于每个Proxy的<B>IObtainable/IModifiable功能</B>了：
...
<B>从初始界面StartupWindow.cs看起：</B>

<B>操作1：创建BindingSet</B>
...
<B>InteractionRequest</B>在Interactivity中已经解释过，是一种<B><VT>简单的事件触发机制</VT></B>
<B>可以推断：</B>
这个绑定并无真正的绑定对象如Button之类的，而是<B><VT>函数与操作的绑定</VT></B>，很明显这里就是<VT>将函数添加到Request事件中，之后即可使用`Raise()`进行触发</VT>

我们对于这个例子需要关心的有3件事：<B>Source/Target/流向</B>
...
以上几个例子可能没有包含所有常用情况，但足以说明<B>绑定的原理</B>了：
<B><VT>有Source以及Target，由`For()`/`To()`确定Source与Target是什么，由`OneWay()`之类的函数确定流向，Source以及Target各自创建自己的Proxy即可，如果是可绑定的则会通过相应的事件机制进行绑定，不可绑定或流向错误则不会绑定</VT></B>
...
<B><GN>Localization</GN></B>是整个本地化的<B>核心</B>，从这里入手是一个比较好的选择，其<B>声明</B>为：
`public class Localization : ILocalization`
了解一下<B>接口</B>，接口操作很多，但可以简单地汇总为以下内容：

- `Subset()`
- `ContainsKey()`
- `GetXXX()`

`Subset()`我们应该很熟悉，这在<B>Configuration</B>中出现过，是一种<B><VT>子集形式</VT></B>
...
现在来看就清晰很多：
<B><VT>对于每个provider(ProviderEntry)来说，都会通过其`provider.Load()`操作获取当前语言情况下的kv组，创建ObservableProperty并更新`entry.Keys`以及存入data(一个cache)中</VT></B>
...
但是此时这点信息不足以显示场景中的Text，这<B>与Text本身的绑定有关</B>：
观察后可以发现：Text上挂载着名为<B>LocalizedText</B>的脚本：
![](Pic/Loxodon10.png)
其<B>声明</B>如下：
...
以上是一般形式，这也意味着本地化本身是独立存在的，无需在MVVM框架上即可运行，但使用<B>MVVM框架形式</B>是完全可行的：
...
检查场景，我们能发现<B>MVVM形式与一般形式的不同</B>：
<B><VT>在Text上，无需再手动添加LocalizedText并输入key进行绑定，由上述语句替代</VT></B>
...
- Source方面：
  - 考虑`localization.AddDataProvider()`，这里的绑定与之前不同：
    ...
    但是无论是哪种方式，最终的目的还是<B>获取kv组</B>而已
...
<B>以上可以说是有关绑定最常见的2种情况，但还有一些其它的需要考虑：</B>
...
该类与前面分析的如LocalizedString不是一种内容，这里是<B>不具有Monobehaviour</B>的
...
显然：<B>key指的是语言类型，value指的是对应string</B>
...
具体的实现在<B><GN>CodeGenerator</GN></B>，根据上述例子，进行<B>字符串拼接</B>即可完成

对于使用来说，更重要的是<B><GN>V</GN></B>，<B>声明</B>如下：
`public class V<T> : IObservableProperty<T>`
这区别于一般情况下使用的ObservableProperty，显然这会是一种<B><VT>本地化特化</VT></B>
<B><YL>以Res的一个绑定为例：</YL></B>
...
<B>所以说：</B>
<B><VT>V就是一种类似于ObservableObject的机制，会针对本地化进行自动绑定(但在此之前需要先生成一下V)</VT></B>
