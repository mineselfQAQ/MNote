<center><B><BBBG>CatLib分析</BBBG></B></center>
...
这里很清楚地看到，summary提到这里是<B>项目主入口</B>
该类中做的仅是override，实现了一些操作：

- `OnStartCompleted()`：初始化完成后回调
- `GetBootstraps()`

<B>Framework.cs</B>是定义处：
...
使用反编译可看到CatLib.Core.dll的<B>结构</B>：
![](./Pic/CatLib3.png)
真的是<B><VT>非常的精简</VT></B>
...
可以看到Application其实是<B>继承于Container</B>的
观察后，可以发现我们应该这么理解：
<B><VT>Application具有IApplication的功能，同时又具有IContainer的功能</VT></B>

#### Container

Container继承了IContainer接口，内容很多，简单<B>整理</B>如下：

- <B>服务绑定</B> `Bind()` `BindIf()` `Unbind()` `GetBind()` `HasBind()`
- <B>服务解析</B> `Make()` `this[]` `CanMake()` `IsResolved()` `HasInstance()`
- <B>生命周期</B> `Instance()` `Release()` `Flush()` `IsStatic()`
- <B>方法注入</B> `BindMethod()` `UnbindMethod()` `Invoke()` `Call()`
- <B>服务标记</B> `Tag()` `Tagged()`
- <B>别名系统</B> `Alias()` `IsAlias()`
- <B>扩展事件</B> `Extend()` `OnResolving()` `OnAfterResolving()` `OnRelease()` `OnRebound()`
- <B>类型处理</B> `OnFindType()` `Type2Service()`
- <B>依赖添加</B> `Needs()` `Given()` <VT>需连写</VT>
...
<B><GN>Guard</GN></B>是CatLib的一个<B>Utility类</B>，用于<B><VT>代码安全相关</VT></B>，如`ParameterNotNull()`就是用于保证传参的正确性的，同时`GuardServiceName()`/`GuardFlushing()`也是一些防御代码
真正<B>关键</B>的是这一句：`BindData bindData = new BindData(this, service, concrete, isStatic);`
<B><VT>BindData的创建意味着绑定的完成</VT></B>，在此之后根据`IsResolved()`的情况进行了特殊处理
...
<B><GN>Bindable</GN></B>是其基，其<B>接口IBindable</B>具有以下内容：
...
<B>接口IBindData</B>是BindData扩展的内容，有：

- Concrete事件/IsStatic
- Alias()/Tag()/OnResolving()/OnAfterResolving()/OnRelease()

<B>Concrete</B>如下所示：`public Func<IContainer, object[], object> Concrete { get; }`
仅有get方法，这意味着仅能通过构造函数传入
由其名字与返回值可知：<B><VT>是一个用于真正创建IContainer的事件</VT></B>
与Concrete类似的有<B>IsStatic</B>，同通过构造函数传入
<B>其余函数</B>我们<VT>都在Container声明中见过，同时也确实用的就是Container的</VT>
<B>要注意的是：</B>
Bindata使用的是<B>`Bindable<IBindData>`</B>，即类似与Singleton的反向引用
在类中，IBindData泛型被称为<B>TReturn</B>，也就是<B><VT>返回值</VT></B>的含义
...
就这么来看，<B>BindData的创建</B>并未收集到太多信息，收集到的信息核心就只是`Needs()`的service，同时，前面提到的`Given()`此时还未调用
...
- instance的创建有2种方式：
  ...
  - 如果没有Concrete，则会通过这种方式创建：
    ...
    第一件事是<B>判断类型可用性</B>：`this.IsUnableType(makeServiceType)`
    对于abstract/interface/array/enum/可空类型/null是不可用的
    对于其它一般的类型都是可用的
    由于类型是用string表示的，所以需要通过`SpeculatedServiceType()`进行获取，这主要是通过<B>findType</B>(`SortSet<Func<string, Type>, int>`，大致是一种具有优先级的string转Type的事件)
    <B>findType的添加</B>是通过`Container.OnFindType()`完成的，对于Application.cs来说，有<B>默认实现</B>：
    `this.OnFindType((Func<string, Type>) (finder => Type.GetType(finder)), int.MaxValue);`
    同时<B>App.cs也提供了`OnFindType()`的包装</B>
    所以说转换其实很方便，简单来说就是<VT>调用了`Type.GetType()`进行了转换</VT>

    第二件事是<B>获取构造函数参数列表</B>：`this.GetConstructorsInjectParams(makeServiceBindData, makeServiceType, userParams);`
    ...
    最后非常简单，就是<B>创建实例</B>：`this.CreateInstance(makeServiceType, userParams)`
...
- <B><VT>Tip：instance创建与`[Inject]`并不冲突，构造函数可以传统注入一些，而属性注入可以注入另一些</VT></B>
...
<B>extenders的添加</B>在`Extend()`的另一个public声明中完成：
...
可以看到：其实<B>extenders</B>是用于解析时的，如果实例已创建则根本无需extenders的，但同时需要更改一下<B>instancesReverse</B>的对象
...
<B>rebound注册</B>则是通过`OnRebound()`完成
...
<B><BL>问题：`Resolve()`中的BuildStack/UserParamsStack的作用是什么</BL></B>
...
<B>总之：<VT>执行`Make()`则会真正获取到service实例</VT></B>

绑定与解析可以说是DI的核心，<B>其余</B>还存在一些<B>特性</B>：
...
相比`Call()`的<B>一次性调用</B>，<B>`BindMethod()`+`Invoke()`</B>是一种<B>更持久的注册</B>
相对的，这里的处理会更加麻烦：
...
<B><GN>MethodContainer</GN></B>是一个Container下的<B>附属Container</B>，专用于方法绑定
...
可以看到，与Container类似，在MethodContainer中的`Bind()`会创建出<B><GN>MethodBind</GN></B>，其本质与BindData相同，其<B>声明</B>为：
`internal sealed class MethodBind : Bindable<IMethodBind>, IMethodBind, IBindable<IMethodBind>, IBindable`
两者同为Bindable，但具有<B>IMethodBind特性</B>而非IBindData，而该接口仅为一个接口：
...
可以看到其中具有一个<B>特性</B>：
<B><VT>通过`Tagged()`获取到的内容并非string，而是由`Make()`生成的service实例</VT></B>
...
<B><VT>`Bind()`用于绑定，`Make()`用于解析，其余基本都是扩展</VT></B>

<BR>

#### Application流程

<B>回到Application.cs：</B>
回看其声明：
`public class Application : CatLib.Container.Container, IApplication, IContainer`
我们其实能够明白：<B><VT>Application是具有DI功能的</VT></B>
与其说是继承，不如说其实本来就只是一个Application的功能
<B>IApplication接口</B>才是真正提供的功能：
...
这些内容算是非常熟悉的，毕竟这是<B>在Unity层中所需要调用的内容</B>
但同时也存在一些重要函数直接定义在Application中，考虑Framework中的流程：

在Awake阶段执行了<B>Bootstrap系列</B>操作：
首先是`BeforeBootstrap()`：本质上这是在调用`application.GetDispatcher()?.AddListener()`进行回调的添加，所谓的dispatcher即<B><GN>EventDispatcher</GN></B>，总之肯定是<B>某种事件触发机制</B>
...
<B>IBootstrap接口</B>非常简单：
`public interface IBootstrap{void Bootstrap();}`
接口仅需要一个`Bootstrap()`操作，也就意味着：
<B><VT>`Bootstrap()`是Awake时的一个流程，我们需要自行实现若干个IBootstrap以完成Awake流程，具体做什么都是可行的(但DI创建是最关键的)</VT></B>

在Start阶段执行了<B>Init</B>操作：
...
`Register()`需要IServiceProvider，这显然是<B><VT>服务的一层包装</VT></B>，其<B>声明</B>为：
...
但是可以注意到其中有一个不同，就是对<B>instanceTiming</B>的操作`Release()`：
...
以上可以说是框架的全部，除了提到的函数，基本上所有的函数都在<B>Application.cs/Container.cs/ContainerExtension.cs</B>中定义
