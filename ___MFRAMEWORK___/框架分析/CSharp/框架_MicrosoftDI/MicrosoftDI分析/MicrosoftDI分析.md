<center><B><BBBG>MicrosoftDI分析</BBBG></B></center>

<B>参考版本：v3.1.32</B>
<B>[Github](https://github.com/dotnet/extensions/tree/v3.1.32/src/DependencyInjection)</B>
...
可以看到<B>接口IServiceCollection</B>本质上是一个IList<>，即<B><VT>具有列表功能</VT></B>
...
一般来说<B>泛型注册</B>应该会是比较常用的一种形式，毕竟这看起来最方便
...
<B><GN>ServiceLifetime</GN></B>是其中一个参数， 可以看到是构建ServiceDescriptor的一部分，其实是个<B>枚举</B>：
...
<B>要注意：</B>
<B><VT>其中的内容更丰富，但扩展对象依旧是IServiceCollection，所以用法是没有变的</VT></B>
...
<B><GN>ServiceProvider</GN></B><B>声明</B>如下：
`public sealed class ServiceProvider : IServiceProvider, IDisposable, IServiceProviderEngineCallback, IAsyncDisposable`
其中<B>IDisposable/IAsyncDisposable</B>是<B><VT>dispose功能</VT></B>，using语句就是它提供的
<B><VT>Tip：`using`对应IDisposable，`await using`对应IAsyncDisposable</VT></B>

<B>IServiceProviderEngineCallback</B>看起来是有关引擎的回调，内容如下：
...
<B>IServiceProvider</B>则是核心，其中只有一个函数`object GetService(Type serviceType)`
这也是provider的功能：<B><VT>提供service</VT></B>

那么这里先看一下前面没有分析的<B>构造函数</B>：
...
可以看得出这里的2个关键：<B>IServiceProviderEngine与CallSiteValidator</B>
...
所以我们大致可以理解：
<B><VT>ServiceProvider是一个用于获取service的类，这主要通过engine完成(需要CallSiteValidator的配合)</VT></B>
...
CallSiteVisitor一看就是<B><VT>访问者模式</VT></B>，先看<B>构造</B>：
...
构造没什么特别，<B><GN>StackGuard</GN></B>是一个用于<B>栈检查</B>的类，后续会通过它<B><VT>确保栈不会溢出</VT></B>
...
这里最先需要了解的是<B><GN>ServiceCallSite</GN></B>，看起来其实是<B>一组信息</B>，类似Descriptor，看起来都挺熟悉：
...
看一下函数声明，可以了解到：
<B><VT>泛型中，TArgument指的是传入参数，TResult指的是输出结果</VT></B>
...
<B><VT>对于缓存情况，只要派生类实现即可，如没有实现则使用默认创建方式`VisitCallSiteMain()`，其中`VisitNoCache()`虽然使用了virtual，但不会进行override，一定会使用`VisitCallSiteMain()`，对于创建的每一种情况则必须进行override(因为是抽象的)</VT></B>
...
默认情况下，优先选择DynamicServiceProviderEngine，否则会使用RuntimeServiceProviderEngine，显然<B>动态的要更好</B>

这些Engine都有一个<B>基类</B>，为<B><GN>ServiceProviderEngine</GN></B>
其<B>声明</B>为：
`internal abstract class ServiceProviderEngine : IServiceProviderEngine, IServiceScopeFactory`

<B>IServiceProviderEngine接口</B>是一个复合接口，有：
...
而<B>IServiceScopeFactory接口</B>提供了`CreateScope()`
...
构造函数中，我们就能发现<B>许多重要内容</B>：

- _createServiceAccessor：一个双重回调，传入Type，返回一个Func-传入object返回<B>ServiceProviderEngineScope</B>
- Root：就是一个<B>ServiceProviderEngineScope</B>
- RuntimeResolver：<B>CallSiteRuntimeResolver</B>类似CallSiteValidator，另一种CallSiteVisitor派生
- CallSiteFactory：一个<B>CallSiteFactory</B>
...
<B>CallSiteFactory</B>即<B><GN>CallSiteFactory</GN></B>
<B>构造</B>如下：
...
<B><VT>CallSiteChain是一个循环链检查类</VT></B>，后续的传递想必也是为了检查循环

<B>由此我们大致可知：</B>
<B><VT>CallSiteFactory是一个工厂，核心就是在获取ServiceCallSite，具体的细节非常复杂</VT></B>
...
表面上就是填充_scopedServices，这会在后续检测中<B>作为条件</B>：
...
更关键的是<B>`RealizeService()`</B>，这是一个<B>抽象函数</B>，所以每一种Engine都会有自己的实现，最简单的当然是RuntimeServiceProviderEngine：
...
查看其它Engine的实现，其实大差不大，主要是<B>realizedService创建方式不同</B>
...
<B>`VisitConstructor()`</B>想必是<B>构造函数注入</B>，流程如下：
...
可以说以上流程就是流程的全部，对于<B>更复杂的情况</B>，无非就是：
