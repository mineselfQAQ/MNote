<center><B><BBBG>WPFMVVM分析</BBBG></B></center>
...
首先使用的是MVVM，所以框架本身怎么都脱离不了<B>Model/View/ModelView</B>三个内容
其次，由于是基于C#(.NET)，微软本身提供了非常多有用的<B>Extensions</B>，比如说这里最实用的<B><GN>DI(`Microsoft.Extensions.DependencyInjection`)</GN></B>

所以我们先来看一下<B>目录结构</B>：
![](Pic/WPFMVVM1.png)

<B><BL>问题:Test/E2E是什么</BL></B>
<BL>Test我们大概率会知道，即<B><GN>GN</GN></B>，这是比较常见的
而E2E其实也是一种单元测试，只是是针对于<B><GN>GN</GN></B>，即<B><VT>从用户输入到数据用于验证操作执行正确性</VT></B></BL>
...
<B>所以大致其实就是：</B>
<B><VT>子提供数据(可附带比较)，父整合子数据并提供函数</VT></B>

以上三个类都是很好理解的，而剩下的<B>ReservationBook</B>由于使用了<VT>DI包装</VT>，具体理解起来是比较非常复杂的：
...
在WPF中，视图展示就是通过xaml文件完成的，<B>大致就是</B>：
...
在脚本中有所对应：
<B>核心：<VT>继承自`INotifyPropertyChanged`，由`OnPropertyChanged()`触发PropertyChanged
　　　这意味着每当Store中的`CurrentViewModel`改变则会触发</VT></B>
...
与Models一样承担的<B>职责</B>都有所<B>不同</B>：
...
前面也提到过，绑定是WPF的关键，所以<B>详细</B>来<B>分析</B>一下绑定：
比较典型的绑定就是<B>属性绑定</B>了：
...
<B>更重要的是：<VT>所有子元素默认继承父元素的DataContext</VT></B>
<B>这意味着：<VT>该ViewModel下的所有属性都可以自动绑定</VT></B>
...
先看<B>执行</B>：
<B>核心：<VT>ICommand会被存入ViewModel，并在相应函数调用</VT></B>
...
在ICommand中，与INotifyPropertyChanged类似，同样<B>存在绑定</B>
先看上述<B>基础继承链</B>：
在CommandBase中，
...
<B><YL>以MakeReservationCommand为例：</YL></B>
...
<B>调用链：</B>`Username`变更触发`OnPropertyChanged(nameof(CanCreateReservation))`触发`OnViewModelPropertyChanged()`执行`OnCanExecutedChanged()`触发`CanExecuteChanged`事件，执行`CanExecute()`并更新激活性
<B>简述：<VT>Username属性的变更会触发到`OnCanExecutedChanged()`，即执行`CanExecute()`并更新激活性</VT></B>

<B>总结：</B>
<B><VT>结合代码与view来看，Button通常会绑定一个ICommand，核心会执行`Execute()`，
同时通过上述流程可通过`CanExecute()`控制按钮可用性</VT></B>

### Host/Services/Stores

在该例中，<B>Services</B>是很重要的：
...
<B><YL>这里就拿最简单的IReservationProvider为例：</YL></B>
...
<B><BL>问题：为什么要封装一个Store，Hotel本身不够吗？</BL></B>
<BL>确实如此，以一个函数为例：</BL>
...
- `ReservationListingViewModel._reservations`---ViewModel作为真正的视图刷新数据通过`HotelStore._reservations`封装成了<B>ReservationViewModel列表</B>供视图刷新使用

#### Navigation分析

Navigation指的是<B>导航功能</B>，简单来说就是<VT>从界面a跳转至界面b</VT>
与其相关的当然是<B>NavigationService</B>与<B>NavigationStore</B>
先来看一下Service的声明：
...
结合代码可以<B>推论</B>：<B><VT>NavigationService提供了切换ViewModel的服务</VT></B>
...
从该脚本我们就可以看出：
<B><VT>`_navigationStore`是一个管理CurrentViewModel的地方，同时仅存在一个ViewModel，通过`NavigationService.Navigate()`可延迟调用初始化工厂以更新CurrentViewModel</VT></B>

<B>最终存放处：`MainViewModel.CurrentViewModel`<VT>(为了视图更新绑定)</VT></B>
...
绑定带来的最大的一个<B>好处</B>就是<VT><B>低耦合</B>：ViewModel与View不需要有引用(在表面上消失，但实际还是存在的)</VT>

<B>所以说：</B>
<B><VT>以上的分析只能代表MVVM的功能以及拆分方式，而具体的核心双向绑定被WPF隐藏在底层了</VT></B>
...
<B><BL>问题：Models/ViewModels/Stores的区别</BL></B>
<BL>Models是数据本身，ViewModels是View所需的数据
而Stores是一种在Models之上，ViewModels之下的数据，包装Models后唯一存在，供ViewModels使用</BL>

虽然不分析底层，我们无法得知真正的流程，但是我们最起码还是了解了<B>ViewModels与Models/Views的关系，简单来说：</B>
<B><VT>ViewModels是Models的一层包装，其中收集了视图刷新所需的所有数据，数据有2种：</VT></B>
